...
<html>
<head>
    <title>Report - Invoice Form</title>
    <style>
    .footer { 
        position: absolute; 
        bottom: 0; 
...
<body>
    <div class="header">
        <div class="h-1">
            @if (empty($result['custinv'][0]->ref_no))
                {{$result['custinv'][0]->inv_no}}
            @else
                {{$result['custinv'][0]->ref_no}}
            @endif
        </div>
        <div class="h-2">
            <div style="padding-left: 50px;">{{$result['custinv'][0]->inv_no}}</div>
            <div>Faktur Pajak</div>
            <div>PO No.: {{$result['custinv'][0]->note_po}}</div>
        </div>
    </div>
    <div class="customer">
    ...
    <div class="glar">{{$result['custinv'][0]->glar}}</div>
