...
$(document).ready(function () {
    ...
    const get_tbl_item = () => {
        return new Promise(function(resolve, reject) {
            ...
            let tbl_item = $('#do-datatables-items').DataTable({
                ...
                destroy: true,
                ajax: {
                    url: "{{ route('tms.warehouse.do_pending.header_tools') }}",
                    method: "POST",
                    headers: token_header,
                    data: params
                },
    ...
    $('#do-btn-modal-create').on('click', function () {
        modalAction('#do-modal-create').then(function (resolve) {
            ...
            ajax("{{ route('tms.warehouse.do_pending.header_tools') }}",
                "POST",
                params,
                (response) => {
    ...
    $(document).on('click', '#do-btn-create-close', function () {
        ajax("{{ route('tms.warehouse.do_pending.cancel_form') }}",
            "POST",
    ...
    $(document).on('click', '#do-btn-delete-item', function () {
        ...
        ajax("{{route('tms.warehouse.do_pending.delete_item')}}", "POST", params, (response) => {
            response = response.responseJSON;
    ...
    $(document).off('click', '.do-act-view').on('click', '.do-act-view', function () {
        ...
        ajax("{{route('tms.warehouse.do_pending.read')}}", "GET", {"do_no": id, "view_do": true}, (response) => {
            response = response.responseJSON;
    ...
    $(document).on('submit', '#do-form-ng', function () {
        ...
        if (count == 0) {
            ...
            var post = {
                "route": "{{route('tms.warehouse.do_pending.ng')}}",
                "method": "POST",
                "data": data
            };
    ...
    $(document).off('click', '.do-act-voided').on('click', '.do-act-voided', function () {
        var id = $(this).data('dono');
        ajax("{{route('tms.warehouse.do_pending.read')}}", "GET", {"do_no": id, "check": true}, (response) => {
            response = response.responseJSON;
            if (response.message == null) {
                Swal.fire({
                    ...
                    if (answer.value == true) {
                        ajax("{{route('tms.warehouse.do_pending.void')}}", "POST", {"do_no": id}, (response) => {
                            response = response.responseJSON;
                            if (response.status == true) {
                                showNotif({
                                    'title': 'Notification',
    ...
    $(document).off('click', '.do-act-unvoided').on('click', '.do-act-unvoided', function () {
        ...
        Swal.fire({
            ...
            if (answer.value != "" && answer.value != undefined) {
                ...
                ajax("{{route('tms.warehouse.do_pending.unvoid')}}", "POST", {"do_no": id, "note": note}, (response) => {
                    response = response.responseJSON;
                    if (response.status == true) {
                        showNotif({
                            'title': 'Notification',
            ...
            if (response.message == null) {
                Swal.fire({
                    ...
                    if (answer.value != "" && answer.value != undefined) {
                        ...
                        ajax("{{route('tms.warehouse.do_pending.post')}}", "POST", {"do_no": id, "rr_no": rr_no}, (response) => {
                            response = response.responseJSON;
                            if (response.status == true) {
                                showNotif({
                                    'title': 'Notification',
    ...
    $(document).off('click', '.do-act-unposted').on('click', '.do-act-unposted', function () {
        ...
        Swal.fire({
            ...
            if (answer.value != "" && answer.value != undefined) {
                var note = answer.value;
                ajax("{{route('tms.warehouse.do_pending.unpost')}}", "POST", {"do_no": id, "note": note}, (response) => {
                    response = response.responseJSON;
                    if (response.status == true) {
                        showNotif({
                            'title': 'Notification',
    ...
    $(document).off('click', '.do-act-finished').on('click', '.do-act-finished', function () {
        ...
        Swal.fire({
            ...
            if (answer.value != "" && answer.value != undefined) {
                var note = answer.value;
                ajax("{{route('tms.warehouse.do_pending.finish')}}", "POST", {"do_no": id}, (response) => {
                    response = response.responseJSON;
                    if (response.status == true) {
                        showNotif({
                            'title': 'Notification',
    ...
    $(document).off('click', '.do-act-unfinished').on('click', '.do-act-unfinished', function () {
        ...
        Swal.fire({
            ...
            if (answer.value != "" && answer.value != undefined) {
                var note = answer.value;
                ajax("{{route('tms.warehouse.do_pending.unfinish')}}", "POST", {"do_no": id, "note": note}, (response) => {
                    response = response.responseJSON;
                    if (response.status == true) {
                        showNotif({
                            'title': 'Notification',
    ...
    $(document).off('click', '.do-act-report').on('click', '.do-act-report', function () {
        var id = $(this).data('dono');
        ajax("{{route('tms.warehouse.do_pending.read')}}", "GET", {"do_no": id, "check_print": true}, (response) => {
            response = response.responseJSON;
            if (response.message !== null) {
                Swal.fire({
                    title: 'Access Denied!',
    ...
    $(document).on('click', '#do-btn-print-gas', function () {
        if ($('#do-print-dari').val() == "" || $('#do-print-sampai').val() == "") {
            ...
            var url = "{{route('tms.warehouse.do_pending.print')}}?params=" + encrypt;
    ...
    $(document).on('click', '#do-btn-revise', function () {
        if ($('#do-create-date').val() == "") {
            ...
            if ($('#do-create-date').val() == window.localStorage.getItem('date-old')) {
                ...
                var post = {
                    "route": "{{route('tms.warehouse.do_pending.revise')}}",
                    "method": "POST",
                    "data": form_data
                };
                $('#do-btn-revise').prop('disabled', true);
    ...
    $(document).on('submit', '#do-form-create', () => {
        ...
        if (tbl_additem.rows().data().toArray().length > 0) {
            ...
            var params = {
                "route": "{{route('tms.warehouse.do_pending.read')}}",
                "method": "GET",
                "data": {
                    "do_no": $('#do-create-no').val()
                }
            ...
            ajaxWithPromise(params).then(resolve => {
                ...
                if (response.message == 'false') {
                    route = "{{route('tms.warehouse.do_pending.create')}}";
                }else{
                    route = "{{route('tms.warehouse.do_pending.update')}}";
                }
                var post = {
                    "route": route,
                    "method": "POST",
        ...
        modalAction('#do-modal-setting').then((resolve) => {
            resolve.on('shown.bs.modal', () => {
                ...
                tbl_do_setting = $('#do-setting-datatables').DataTable({
                    ...
                    destroy: true,
                    ajax: {
                        url: "{{ route('tms.warehouse.do_pending.table_index_setting') }}",
                        method: 'GET',
                        headers: token_header
                    },
                    columns: column,
    ...
    $(document).on('click', '#do-btn-setting-save', () => {
        ...
        ajax(
            "{{ route('tms.warehouse.do_pending.header_tools') }}",
            "POST",
